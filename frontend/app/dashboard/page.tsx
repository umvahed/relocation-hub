...
export default function DashboardPage() {
  ...
  const copyWhatsAppReminder = (task: any) => {
                  ...
                  {profile?.share_token && (
                    <div className="px-4 py-2.5 border-b border-gray-100 dark:border-gray-700">
                      <button
                        ...
                        Share progress with HR
                      </button>
                      {showShareLink && (
                        <div className="mt-2 space-y-1.5">
                          <p className="text-xs text-gray-400 dark:text-gray-500">Anyone with this link can view your progress — no account needed.</p>
                          <div className="flex gap-1.5">
                            <input
                              readOnly
                              value={`${typeof window !== 'undefined' ? window.location.origin : ''}/share/${profile.share_token}`}
                            ...
                            />
                            <button
                              onClick={() => copyShareLink(profile.share_token)}
                        ...
                        <div className="px-4 pb-4 border-t border-gray-100 dark:border-gray-700 pt-3 space-y-4">
                          ...
                          {task.due_date && (
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              <button
                                onClick={() => copyWhatsAppReminder(task)}
                                className="inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 hover:bg-emerald-100 dark:hover:bg-emerald-900/50 transition"
                              >
                                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
                                </svg>
                                {copiedTaskId === task.id ? 'Copied!' : 'Copy reminder'}
                              </button>
                            </div>
                          )}
