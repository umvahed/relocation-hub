...
const LEVEL_STYLES = {
  low: { bar: 'bg-emerald-500', badge: 'bg-emerald-50 text-emerald-700', label: 'Low risk' },
  med: { bar: 'bg-amber-500',   badge: 'bg-amber-50 text-amber-700',   label: 'Medium risk' },
  high: { bar: 'bg-red-500',    badge: 'bg-red-50 text-red-700',       label: 'High risk' },
}
    ...
    return (
      <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
        <div className="flex items-center justify-between mb-1">
          <h2 className="text-sm font-semibold text-gray-900">Relocation Risk Score</h2>
          <span className="text-xs font-medium px-2.5 py-1 rounded-full bg-indigo-50 text-indigo-600">Paid</span>
        </div>
        <p className="text-xs text-gray-400">Upgrade to see your personalised risk score across 4 dimensions. <span className="font-medium text-indigo-500">Coming soon.</span></p>
      </div>
    )
  }
      ...
      <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            ...
            disabled={loading}
            className="text-xs font-medium px-3 py-1.5 rounded-lg bg-indigo-50 text-indigo-600 hover:bg-indigo-100 transition disabled:opacity-50 flex items-center gap-1.5"
          >
            {loading && (
        ...
        {!score && !loading && (
          <p className="text-xs text-gray-400">Click "Compute score" to analyse your relocation readiness across 4 dimensions.</p>
        )}
        ...
        {score && s && (
          <div className="space-y-4">
            ...
            <div className="flex items-end gap-3">
              <span className="text-4xl font-bold text-gray-900 leading-none">{score.score}</span>
            ...
            <div className="grid grid-cols-2 gap-2">
              {Object.entries(score.dimension_scores).map(([key, val]) => (
                <div key={key} className="bg-gray-50 rounded-xl px-3 py-2">
                  <p className="text-xs text-gray-500">{DIM_LABELS[key] ?? key}</p>
            ...
            {score.risk_items.length > 0 && (
              <div className="space-y-2 pt-1">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Top risks to address</p>
                {score.risk_items.map(item => (
                  <div key={item.rank} className="flex gap-3 bg-gray-50 rounded-xl px-3 py-2.5">
                    <span className="text-xs font-bold text-gray-300 mt-0.5 flex-shrink-0">#{item.rank}</span>
                    <div>
                      <p className="text-xs font-semibold text-gray-800">{item.title}</p>
                      <p className="text-xs text-gray-500 mt-0.5">{item.detail}</p>
                      <p className="text-xs text-indigo-600 mt-0.5 font-medium">{item.action}</p>
                    </div>
                  </div>
                ))}
              </div>
