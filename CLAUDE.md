...
| `CRON_SECRET` | Railway (verified by `verify_cron_secret()` in deps.py — all cron endpoints require `Authorization: Bearer <value>`) |
| `RAILWAY_URL` | GitHub Actions secret (IND weekly reset cron calls Railway directly) |
| `CRON_SECRET` | GitHub Actions secret (IND weekly reset sends `Authorization: Bearer <CRON_SECRET>`) |

Never put `NEXT_PUBLIC_*` in Railway. Never put `FRONTEND_URL` in Vercel. `RESEND_API_KEY` lives in BOTH Vercel (cron proxies) and Railway (email sending). `CRON_SECRET` lives in Railway + GitHub Actions only — cron-job.org jobs send it as a request header (set per-job in the cron-job.org UI under Headers, NOT in their API Keys settings).
...
- CORS in `backend/app/main.py` allows `valryn.nl`, `www.valryn.nl`, and `localhost:3000` (`relocation-hub.vercel.app` removed — DNS transition complete)
- **Auth middleware**: `get_current_user_id()` in `backend/app/deps.py` verifies Supabase JWT on all user-facing endpoints. `verify_cron_secret()` protects all cron endpoints. `verify_admin_secret()` protects admin endpoints.
- **OpenAPI docs** (`/docs`, `/redoc`) are disabled in production — only available when `ENVIRONMENT=development`
...
| POST | `/api/feedback` | Public — save user feedback (source, most_useful, missing, nps_score, name, email); emails Ahmed via Resend on each submission |
...
- GitHub Actions secrets: `RAILWAY_URL` + `CRON_SECRET` set; `ind_monitor.yml` runs every Monday
...
- Supabase: migration 017 run (RLS on `ind_appointments` + `increment_promo_uses` RPC for atomic promo counter)
- Supabase: migration 018 run (`feedback` table with RLS — service role only)
- Railway: `CRON_SECRET` env var set
- GitHub Actions secrets: `RAILWAY_URL` + `CRON_SECRET` set (`RESEND_API_KEY` secret removed — no longer used by workflows)
- cron-job.org: all 4 jobs updated to send `Authorization: Bearer <CRON_SECRET>` request header
- Google Ads: tag AW-18163704157 live in `frontend/app/layout.tsx`; CookieYes consent banner active; enhanced conversions enabled
