    ...
    for task in tasks_res.data or []:
        ...
        <div style="font-family: -apple-system, sans-serif; max-width: 560px; margin: 0 auto; padding: 32px 24px; color: #1a1a1a;">
          ...
          <div style="background: #fff7ed; border: 1px solid #fed7aa; border-radius: 12px; padding: 16px 20px; margin-bottom: 24px;">
            ...
            <div style="font-size: 15px; font-weight: 600; color: #9a3412;">{task['title']}</div>
            {f'<p style="font-size: 13px; color: #4b5563; margin: 8px 0 0; line-height: 1.5;">{task["description"][:200]}{"..." if len(task["description"]) > 200 else ""}</p>' if task.get("description") else ""}
          </div>
...
async def set_due_date(task_id: str, due_date: str):
    try:
        supabase = get_supabase()
