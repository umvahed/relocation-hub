...
def _build_cover_pdf(profile: dict, documents: list[dict], validations: dict, failed_docs: list[dict] | None = None) -> bytes:
    ...
    pdf.set_text_color(156, 163, 175)
    pdf.cell(0, 5, "Generated by Valryn · valryn.com", align="C", ln=True)
...
async def _build_merged_pdf(user_id: str) -> tuple[bytes, str]:
    ...
    return output.getvalue(), f"Valryn_DocPack_{full_name}.pdf"
...
async def send_docpack_to_hr(user_id: str):
    ...
    <div style="font-family: -apple-system, sans-serif; max-width: 560px; margin: 0 auto; padding: 32px 24px; color: #1a1a1a;">
      ...
      <p style="color: #9ca3af; font-size: 11px; margin: 0;">Sent via Valryn &middot; valryn.com</p>
    </div>
    """
